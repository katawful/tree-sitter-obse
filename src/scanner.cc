...
struct Scanner {
  ...
  void advance(TSLexer *lexer) {
	...
	unsigned serialize(char *buffer) {
		// if the length of our function is bigger than our buffer size, return 0
		if (function.length() >= TREE_SITTER_SERIALIZATION_BUFFER_SIZE) return 0;
		// copy the contents of the buffer character into our string
		function.copy(&buffer[0], function.length());
	...
	void deserialize(const char *buffer, unsigned length) {
		...
		if (length == 0) {
			...
			function.assign(&buffer[0], &buffer[length]);
		}
	}

  bool scan(TSLexer *lexer, const bool *valid_symbols) {
    // we only want to look for a function
    if (valid_symbols[FUNCTION]) {
      // lookahead with the lexer until we hit a non-alphanumeric character
      string currentLook = "";
      // we need to see there's whitespace before the first character
      // if there is still white space, just advance lexer
      char lastChar;
      while (iswalpha(lexer->lookahead) || iswdigit(lexer->lookahead) || iswspace(lexer->lookahead) || lexer->lookahead == '_') {
        if (iswalpha(lexer->lookahead) || iswdigit(lexer->lookahead) || lexer->lookahead == '_') {
          // set lookahead to lower
          currentLook += tolower(lexer->lookahead);
        ...
        if (iswspace(lexer->lookahead)) {
          if (iswalpha(lastChar) || iswdigit(lastChar) || lexer->lookahead == '_') {
            /* advance(lexer); */
            break;
          } else {
            skip(lexer);
      ...
      // put some options where the scanner shouldn't look
      string currentLookTest = currentLook;
      for (char c : currentLookTest) {
        tolower(c);
      }
      if (currentLookTest == "end" ||
          currentLookTest == "let" ||
          currentLookTest == "set" ||
          currentLookTest == "testexpr" ||
          currentLookTest == "eval" ||
          currentLookTest == "seteventhandler" ||
          currentLookTest == "removeeventhandler" ||
          currentLookTest == "return" ||
          currentLookTest == "setfunctionvalue" ||
          currentLookTest == "call") {
        return false;
      }
      lexer->mark_end(lexer);
      if (functionList.find(currentLook) != functionList.end()) {
	...
	// this is what i use to store shit idk this is confusing
	string function;
	// this holds all of the obse functions
	// i use set for speed
...
void *tree_sitter_obse_external_scanner_create() {
	return new Scanner();
	/* return NULL; */
}

bool tree_sitter_obse_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
	Scanner *scanner = static_cast<Scanner *>(payload);
	return scanner->scan(lexer, valid_symbols);
}

unsigned tree_sitter_obse_external_scanner_serialize(void *payload, char *state) {
	Scanner *scanner = static_cast<Scanner *>(payload);
	return scanner->serialize(state);
}

void tree_sitter_obse_external_scanner_deserialize(void *payload, const char *state, unsigned length) {
	Scanner *scanner = static_cast<Scanner *>(payload);
	scanner->deserialize(state, length);
}

void tree_sitter_obse_external_scanner_destroy(void *payload) {
	Scanner *scanner = static_cast<Scanner *>(payload);
	delete scanner;
}
